...
import com.coelacanth9.simplecustomlauncher.data.LayoutState
...
/**
 * ホーム画面の ViewModel。
 * - LayoutState は ShortcutRepository の StateFlow を購読（直接 observe）
...
class HomeViewModel(
    ...
    fun navigateToSlotEdit(pageIndex: Int, row: Int, column: Int) {
        navDestination = NavDestination.SlotEdit(pageIndex, row, column)
    }
    ...
    fun launchShortcut(context: Context, item: ShortcutItem, shortcutHelper: ShortcutHelper) {
        ...
        try {
            when (item.type) {
                ...
                ShortcutType.SETTINGS -> navigateTo(NavDestination.AppSettings)
